...
public class GenericsExample {
    ...
    public static void main(String[] args) {
        Recipe<Ingredient, Cake> recipe = new Recipe<>(new Egg()) // new Flour()도 가능.
        {
            @Override
            public Cake cook() {
    ...
    // 예를 들어, 이 클래스에서는 T를 재료 클래스로, R을 반환 클래스로 사용하고 있습니다.
    // 다만, 이 클래스에서는 제너릭스를 확정하지 않았음으로 클래스를 생성하기 전까지는 T와 R이 무엇인지 알 수 없습니다.
    ...
    static class RottenEgg implements Ingredient {
        @Override
        public void mix() {
            System.out.println("Rotten egg는 잘못된 재료입니다. 다시 시도하세요.");
        }
    }
